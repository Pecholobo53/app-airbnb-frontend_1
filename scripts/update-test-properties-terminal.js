// scripts/update-test-properties-terminal.js
/**
 * Script para ejecutar desde terminal
 * Uso: node scripts/update-test-properties-terminal.js TU_TOKEN_AQUI
 * 
 * Para obtener el token:
 * 1. Abre http://localhost:3001/login en el navegador
 * 2. Inicia sesión
 * 3. Abre la consola (F12)
 * 4. Ejecuta: JSON.parse(sessionStorage.getItem("airbnb_session")).accessToken
 * 5. Copia el token y úsalo como argumento
 */
...
async function updateTestProperties(token) {
  if (!token) {
    ...
    console.log('5. Copia el token y ejecuta: node scripts/update-test-properties-terminal.js TU_TOKEN');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log('📝 Lista de todas las propiedades:');
    console.log('='.repeat(60));
    properties.forEach((prop, index) => {
      const isTest = isTestProperty(prop.title || '');
      const marker = isTest ? '🎯' : '  ';
      console.log(`${marker} ${index + 1}. "${prop.title || 'Sin título'}"`);
    });
    console.log('');
    ...
    console.log('Propiedades que se actualizarán:');
    ...
    for (const property of testProperties) {
      ...
      console.log(`🔄 Actualizando: "${property.title}"`);
...
const token = process.argv[2] || process.env.AIRBNB_TOKEN;

// Ejecutar
updateTestProperties(token).catch(error => {
