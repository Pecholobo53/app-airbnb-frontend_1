      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4">
          {QUICK_FILTERS.map((filter) => {
            ...
            const isHovered = hoveredFilter === filter.label;
            
            return (
              <div
                key={filter.label}
                className="relative"
                onMouseEnter={() => setHoveredFilter(filter.label)}
              ...
              >
                ...
                >
                  ...
                  {/* Tooltip con información adicional - aparece al hover */}
                  {isHovered && (
                    <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-4 py-2 bg-gray-900 text-white text-xs rounded-lg shadow-xl z-20 w-56 text-center animate-in fade-in slide-in-from-bottom-2 duration-200">
                      <p className="font-semibold mb-1 text-[#FF385C]">{filter.label}</p>
                      <p className="text-gray-300">{filter.description}</p>
                      <p className="text-[#FF385C] mt-2 text-[10px] font-medium">Click para ver resultados →</p>
